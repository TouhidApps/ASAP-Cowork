...
class GeneralPurposeAgent(private val providers: LlmProviderRegistry) : Agent {
    ...
    override fun execute(task: Task, context: ProjectContextView): Flow<AgentEvent> = flow {
        ...
        val systemPrompt = buildString {
            ...
            appendLine("You are only one of several specialized agents in this app, each with its own tools and integrations you can't see or use from here. If earlier turns in this conversation show something being done (e.g. Gmail/Sheets/Docs/Calendar being read or edited) that isn't in your own tool list, another agent did that with its own real integration — never claim it didn't happen, was fabricated, or that \"this workspace has no such integration\" just because you personally lack it. If the current request also looks like it belongs to one of those other capabilities rather than a generic file/shell task, say so briefly and suggest the user rephrase to make that clearer, instead of asserting a limitation that may not actually exist.")
